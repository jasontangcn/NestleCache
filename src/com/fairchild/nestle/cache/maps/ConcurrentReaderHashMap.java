...
package com.fairchild.nestle.cache.maps;
 ...
 * <em>not</em> throw ConcurrentModificationException. However, these iterators
 * are designed to be used by only one thread at a time. Sharing an iterator
 * across multiple threads may lead to unpredictable results if the table is
 * being concurrently modified. Again, you can ensure interference-free
...
public class ConcurrentReaderHashMap extends AbstractMap implements Map, Cloneable, Serializable {
	 ...
	 * 
	 * Read operations first proceed without locking, by traversing the apparently
	 * correct list of the apparently correct bin. If an entry is found, but not
	 * invalidated (value field null), it is returned. If not found, operations
	 * must recheck (after a memory barrier) to make sure they are using both the
	 * right list and the right table (which can change under resizes). If
	 * invalidated, reads must acquire main update lock to wait out the update,
	 * and then re-traverse.
	 * 
	 * All list additions are at the front of each bin, making it easy to check
	 * changes, and also fast to traverse. Entry next pointers are never assigned.
	 * Remove() builds new nodes when necessary to preserve this.
	 * 
	 * Remove() (also clear()) invalidates removed nodes to alert read operations
	 * that they must wait out the full modifications.
	 */
	...
	/**
	 * Get ref to table; the reference and the cells it accesses will be at least
	 * as fresh as from last use of barrierLock
	 **/
	 ...
	 * is worth the effort to improve hashcode via the same multiplicative scheme
	 * as used in IdentityHashMap.
	 */
	 ...
	 *          the initial capacity The actual initial capacity is rounded to the
	 *          nearest power of two.
	 * @param loadFactor
	 *          the load factor of the ConcurrentReaderHashMap
	 * @throws IllegalArgumentException
	 *           if the initial maximum number of elements is less than zero, or
	 ...
	 * 16 (whichever is greater), and a default load factor.
	 */
	...
	public Object get(Object key) {
		...
		/*
		 * Start off at the apparently correct bin. If entry is found, we need to
		 * check after a barrier anyway. If not found, we need a barrier to check if
		 * we are actually in right bin. So either way, we encounter only one
		 * barrier unless we need to retry. And we only need to fully synchronize if
		 * there have been concurrent modifications.
		 */
	 ...
	 * this table. Neither the key nor the value can be <code>null</code>.
	 * <p>
	 *
	...
	/**
	 * Continuation of put(), called only when synch lock is held and interference
	 * has been detected.
	 **/
	...
	protected void rehash() {
		 ...
		 * power-of-two expansion, the elements from each bin must either stay at
		 * same index, or move to oldCapacity+index. We also eliminate unnecessary
		 * node creation by catching cases where old nodes can be reused because
		 * their next fields won't change. Statistically, at the default threshhold,
		 * only about one-sixth of them need cloning. (The nodes they replace will
		 * be garbage collectable as soon as they are no longer referenced by any
	...
	/**
	 * Removes the key (and its corresponding value) from this table. This method
	...
	public Object remove(Object key) {
		 ...
		 * 2. Rebuild the list without this entry. All entries following removed
		 * node can stay in list, but all preceeding ones need to be cloned.
		 * Traversals rely on this strategy to ensure that elements will not be
	...
	protected static class Entry implements Map.Entry {
		 ...
		 * changes without synchronization. The other fields are never changed, and
		 * are marked as final.
		 */
		...
		/**
		 * Get the value. Note: In an entrySet or entrySet.iterator, unless the set
		 * or iterator is used under synchronization of the table as a whole (or you
		 * can otherwise guarantee lack of concurrent modification),
		 * <tt>getValue</tt> <em>might</em> return null, reflecting the fact that
		 * the entry has been concurrently removed. However, there are no assurances
		...
		/**
		 * Set the value of this entry. Note: In an entrySet or entrySet.iterator),
		 * unless the set or iterator is used under synchronization of the table as
		 * a whole (or you can otherwise guarantee lack of concurrent modification),
		 * <tt>setValue</tt> is not strictly guaranteed to actually replace the
		 * value field obtained via the <tt>get</tt> operation of the underlying
		 * hash table in multithreaded applications. If iterator-wide
		 * synchronization is not used, and any other concurrent <tt>put</tt> or
		 * <tt>remove</tt> operations occur, sometimes even to <em>other</em>
		 * entries, then this change is not guaranteed to be reflected in the hash
		 * table. (It might, or it might not. There are no assurances either way.)
		 *
		 * @param value
		 *          the new value.
	...
	protected class HashIterator implements Iterator, Enumeration {
		...
		public boolean hasNext() {
			...
			/*
			 * currentkey and currentValue are set here to ensure that next() returns
			 * normally if hasNext() returns true. This avoids surprises especially
			 * when final element is removed during traversal -- instead, we just
			 * ignore the removal during current traversal.
			 */
	...
	/**
	 * Save the state of the <tt>ConcurrentReaderHashMap</tt> instance to a stream
	 * (i.e., serialize it).
	 *
	 * @serialData The <i>capacity</i> of the ConcurrentReaderHashMap (the length
	...
	/**
	 * Reconstitute the <tt>ConcurrentReaderHashMap</tt> instance from a stream
	 * (i.e., deserialize it).
	 */
