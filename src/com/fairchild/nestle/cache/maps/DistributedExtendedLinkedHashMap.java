 ...
 */
public class DistributedExtendedLinkedHashMap extends ExtendedLinkedHashMap implements MessageListener, MembershipListener {
	...
	private final transient Vector notifs = new Vector(); // to be notified
	                                                      // when mbrship
	                                                      // changes
	private final transient Vector members = new Vector(); // keeps track of
	                                                       // all DHTs
	 ...
	 */
	public DistributedExtendedLinkedHashMap(String groupname, ChannelFactory factory, String properties, long state_timeout, int initialCapacity, float loadFactor, boolean accessOrder, int maxinalCapability) throws ChannelException {
		super(initialCapacity, loadFactor, accessOrder, maxinalCapability);
	 ...
	 */
	public DistributedExtendedLinkedHashMap(String groupname, ChannelFactory factory, String properties, boolean persistent, long state_timeout, int initialCapacity, float loadFactor, boolean accessOrder, int maxinalCapability) throws ChannelException {
		super(initialCapacity, loadFactor, accessOrder, maxinalCapability);
		this.groupname = groupname;
	...
	public DistributedExtendedLinkedHashMap(JChannel channel, long state_timeout, int initialCapacity, float loadFactor, boolean accessOrder, int maxinalCapability) throws ChannelNotConnectedException, ChannelClosedException {
		this(channel, false, state_timeout, initialCapacity, loadFactor, accessOrder, maxinalCapability);
	}

	public DistributedExtendedLinkedHashMap(JChannel channel, boolean persistent, long state_timeout, int initialCapacity, float loadFactor, boolean accessOrder, int maxinalCapability) throws ChannelNotConnectedException, ChannelClosedException {
		super(initialCapacity, loadFactor, accessOrder, maxinalCapability);
	 ...
	 *          Max number of milliseconds to wait until state is retrieved
	 */
	public DistributedExtendedLinkedHashMap(PullPushAdapter adapter, Serializable id, long state_timeout, int initialCapacity, float loadFactor, boolean accessOrder, int maxinalCapability) throws ChannelNotConnectedException, ChannelClosedException {
		super(initialCapacity, loadFactor, accessOrder, maxinalCapability);
		initSignatures();
	...
	public DistributedExtendedLinkedHashMap(PullPushAdapter adapter, Serializable id, int initialCapacity, float loadFactor, boolean accessOrder, int maxinalCapability) {
		super(initialCapacity, loadFactor, accessOrder, maxinalCapability);
		initSignatures();
		this.channel = (Channel) adapter.getTransport();
		this.groupname = this.channel.getChannelName();
	...
	protected void init(long state_timeout) throws ChannelClosedException, ChannelNotConnectedException {
		...
		// when providing
		// own channel). First, Channel.connect() has to be called, then
		// start().
		// start(state_timeout);
	}
	...
	public Object put(Object key, Object value) {
		...
		if (send_message == true) {
			...
			_put(key, value);
			// don't have to do prev_val = super.put(..) as is done at the
			// beginning
		}
		return prev_val;
	}
	...
	public Object remove(Object key) {
		...
		if (send_message == true) {
			...
			_remove(key);
			// don't have to do retval = super.remove(..) as is done at the
			// beginning
		}
		return retval;
	}
	...
	public byte[] getState() {
		...
		/* LinkedHashset的Fast-Fail很有特色 */
		/*
		 * for(Iterator iter=keySet().iterator(); iter.hasNext();) {
		 * key=iter.next(); val=get(key); copy.put(key, val); }
		 */
		Set entrys = entrySet();
	...
	public void viewAccepted(View new_view) {
		...
		if (new_mbrs != null) {
			sendViewChangeNotifications(new_mbrs, members); // notifies
			                                                // observers
			                                                // (joined, left)
	...
	/*
	 * public static void main(String[] args) { try { // The setup here is kind of
	 * weird: // 1. Create a channel // 2. Create a DistributedHashtable (on the
	 * channel) // 3. Connect the channel (so the HT gets a VIEW_CHANGE) // 4.
	 * Start the HT // // A simpler setup is // DistributedHashtable ht = new
	 * DistributedHashtable("demo", null, //
	 * "file://c:/JGroups-2.0/conf/state_transfer.xml", 5000);
	 * 
	 * JChannel c = new JChannel( "file:/c:/JGroups-2.0/conf/state_transfer.xml");
	 * c.setOpt(Channel.GET_STATE_EVENTS, Boolean.TRUE); DistributedHashtable ht =
	 * new DistributedHashtable(c, false, 5000); c.connect("demo");
	 * ht.start(5000);
	 * 
	 * ht.put("name", "Michelle Ban"); Object old_key = ht.remove("name");
	 * System.out.println("old key was " + old_key); ht.put("newkey", "newvalue");
	 * 
	 * Map m = new HashMap(); m.put("k1", "v1"); m.put("k2", "v2");
	 * 
	 * ht.putAll(m);
	 * 
	 * System.out.println("hashmap is " + ht); } catch (Throwable t) {
	 * t.printStackTrace(); } }
	 */
}
