...
package com.fairchild.nestle.cache.test;
...
import com.fairchild.nestle.cache.maps.DistributedExtendedLinkedHashMap;

/**
 * Uses the DistributedHashtable building block. The latter subclasses
 * java.util.Hashtable and overrides the methods that modify the hashtable (e.g.
 * put()). Those methods are multicast to the group, whereas read-only methods
 * such as get() use the local copy. A DistributedHashtable is created given the
 * name of a group; all hashtables with the same name find each other and form a
 * group.
 * 
 * @author Bela Ban
 */
public class DistributedHashtableDemo extends Frame implements WindowListener, ActionListener, DistributedExtendedLinkedHashMap.Notification {
	static final String groupname = "HashDemo";
	DistributedExtendedLinkedHashMap h = null;
	...
	private void showAll() {
		...
		for (Iterator en = h.keySet().iterator(); en.hasNext();) {
			key = (String) en.next();
			val = (Float) h.get(key);
			if (val == null)
				continue;
	...
	public void start(ChannelFactory factory, String props, boolean persist) throws ChannelException {
		h = new DistributedExtendedLinkedHashMap(groupname, factory, props, persist, 10000, 100, .80f, true, 200);
		h.addNotifier(this);
	...
	public static void main(String args[]) {
		 ...
		 * + // "DISCARD(down=0.1):" + // this is for discarding of 10% of the up
		 * messages ! "pbcast.PBCAST(gossip_interval=5000;gc_lag=50):" + "UNICAST:"
		 * + "FRAG:" + "pbcast.GMS:" + "pbcast.STATE_TRANSFER";
		 */

		String props = "UDP(mcast_addr=228.8.8.8;mcast_port=45566;ip_ttl=32;" + "mcast_send_buf_size=150000;mcast_recv_buf_size=80000):" + "PING(timeout=2000;num_initial_members=3):" + "MERGE2(min_interval=5000;max_interval=10000):" + "FD_SOCK:" + "VERIFY_SUSPECT(timeout=1500):" + "pbcast.NAKACK(gc_lag=50;retransmit_timeout=300,600,1200,2400,4800):" + "UNICAST(timeout=5000):" + "pbcast.STABLE(desired_avg_gossip=20000):" + "FRAG(frag_size=4096;down_thread=false;up_thread=false):"
		    + "pbcast.GMS(join_timeout=5000;join_retry_timeout=2000;" + "shun=false;print_local_addr=true):" + "pbcast.STATE_TRANSFER";
