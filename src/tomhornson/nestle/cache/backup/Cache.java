 ...
 */
package tomhornson.nestle.cache.backup;
...
public class Cache {
	/*
	 * 使用ExtendLinkedHasMap，保证缓存最大容量。 当objectsCached超过规定容量时，会删除最近最少访问的对象。
	...
	private Map objectsCached = null;
	...
	static {
		...
		if (configFiles.length == 0)
			throw new RuntimeException("Can not get any config file for memory based cache.");

		for (int i = 0; i < configFiles.length; i++) {
			CacheConfig[] config = CacheConfigLoader.parseConfigs(configFiles[i]);
			for (int j = 0; j < config.length; j++) {
				caches.put(config[j].get_name(), new Cache(config[j]));
			}
		}
	}
	...
	public CacheConfig config = null;
